SELECT orders.status_ready,
       CASE
           WHEN COUNT(*) FILTER (WHERE ordersnom.status_cal = 'К' AND ordersnom.kolvo > 0 AND NOT orders.status_ready) >
                0 THEN 'К'
           ELSE ' '
           END                     AS status_cal,
       CASE
       ...
       CASE
           WHEN COUNT(*) FILTER (WHERE ordersnom.status_metall = 'М' AND ordersnom.kolvo > 0) > 0 OR
                NOT dbo.st_set(orders.id) THEN 'М'
           ELSE ' '
           END                     AS status_metall,
       CASE
           WHEN COUNT(*) FILTER (WHERE ordersnom.status_kp = 'КП' AND ordersnom.kolvo > 0 AND NOT orders.status_ready) >
                0
               AND COUNT(*) FILTER (WHERE ordersnom.kolvo > 0 AND
                                          (ordersnom.status_cal = 'К' OR ordersnom.status_instr = 'И' OR
                                           ordersnom.status_draft = 'Ч' OR ordersnom.status_metall = 'М')) = 0
...
FROM dbo.clients AS clients
         ...
         JOIN dbo.tools ON cal_info.tool_id = tools.id -- Join with the tools table
WHERE 1 = 1
/* Dynamic WHERE clause will be added here */
GROUP BY orders.order_manager, orders.status_ready, orders.id, orders.date, clients.name,
         orders.client_id, orders.locked, orders.is_complete, orders.count_pos, orders.comments, orders.goz, tools.name
